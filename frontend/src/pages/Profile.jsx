            ...
            <div key={order._id} style={{ background: '#09090b', padding: '20px', borderRadius: '12px', border: '1px solid #27272a', display: 'flex', flexDirection: 'column', gap: '15px' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ 
                  ...
                  padding: '8px 16px', borderRadius: '20px', fontWeight: 'bold', alignSelf: 'flex-start'
                }}>
                  {order.status}
                </span>
              </div>
              ...
              <div style={{ borderTop: '1px solid #27272a', paddingTop: '12px' }}>
                <p style={{ color: '#a1a1aa', fontSize: '0.85rem', marginBottom: '10px', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Items Ordered</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {order.items.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#18181b', padding: '10px 14px', borderRadius: '8px', border: '1px solid #27272a' }}>
                      <span style={{ color: '#fff', fontWeight: '500', fontSize: '0.95rem' }}>
                        {item.productID?.name || 'Product'}
                      </span>
                      <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                        <span style={{ color: '#a1a1aa', fontSize: '0.9rem' }}>Qty: <strong style={{ color: '#fff' }}>{item.qty}</strong></span>
                        <span style={{ color: '#f97316', fontWeight: '600', fontSize: '0.95rem' }}>₹{item.price.toFixed(2)}</span>
                      </div>
                    </div>
                  ))}
                </div>
